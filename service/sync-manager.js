...
async function syncBaseTables() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const payload = {
        text: `ERROR : ${
          e.error.sqlMessage ? e.error.sqlMessage : defaultErrorMsg
        } : Query : ${e.sqlQuery}`,
      };
      slackService
        .postChannelMesssage(payload)
        .then((res) => {
...
function queuePatients() {
  return new Promise((resolve, reject) => {
    const queues = [
      queueService.generateHivSummarySyncQueue(),
...
function nightlyUpdates() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const payload = {
        text: `ERROR : ${
          e.error.sqlMessage ? e.errorsqlMessage : defaultErrorMsg
        } : Query : ${e.sqlQuery}`,
      };
      slackService
        .postChannelMesssage(payload)
